...
#include "TrackGroupSampler.hlsl"
...
bool SampleAnimation(AnimationClip ac, uint baseAddress, float2 animTime, int rigBoneIndex, uint rigBoneHash, int blendMode, HumanRotationData hrd, out BoneTransformAndFlags btf)
{
    ...
    int trackGroupIndex = QueryPerfectHashTable(rigBoneHash, tsClip.trackGroupPHTSeed, tsClip.trackGroupPHTOffset, tsClip.trackGroupPHTSizeMask);
...
float CalculateFinalLayerWeights(out float layerWeights[8], AnimationJob aj, int humanBodyPart, int boneIndex)
{
    int layerIndex = -1;
    float w = 1.0f;
    ...
    for (int i = aj.animationsToProcessRange.y - 1; i >= 0; --i)
    {
        ...
        float layerWeight = inAvatarMask ? a.layerWeight : 0;
...
void ProcessAnimations(uint tid: SV_DispatchThreadID)
{
    ...
    float refPoseWeight = CalculateFinalLayerWeights(layerWeights, animationJob, rigBone.humanBodyPart, boneWorkload.boneIndexInRig);
    ...
    float currentCumulativeLayerWeight = 1;
